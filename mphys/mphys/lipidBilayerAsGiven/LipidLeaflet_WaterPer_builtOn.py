...
class LipidLeafletWithWaterPer(LipidLeaflet):
    ...
    def d2o_mol_fraction_head(self):
        return (1/self.D2O-self.H2O)*((self.head_solvent.real*27.64)-self.H2O)

    def d2o_mol_fraction_tail(self):
        return (1/self.D2O-self.H2O)*((self.tail_solvent.real*27.64)-self.H2O)

    def wMol_head(self):
        return (self.d2o_mol_fraction_head() * self.D2O) + ((1-self.d2o_mol_fraction_head())*self.H2O)

    def wMol_tail(self):
        return (self.d2o_mol_fraction_tail() * self.D2O) + ((1-self.d2o_mol_fraction_tail())*self.H2O)

    def head_solvent_true(self):
        return float(self.wMol_head()*self.waters_per_head.value/self.vm_head())

    def tail_solvent_true(self):
        return float(self.wMol_tail()*self.waters_per_tail.value/self.vm_tail())
